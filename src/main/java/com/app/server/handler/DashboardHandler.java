...
//                routingContext.put(AppParams.RESPONSE_MSG, HttpResponseStatus.UNAUTHORIZED.reasonPhrase());
                ...
                List<Shipments> listShipments = (List<Shipments>) clipServices.findAllByProperty("FROM Shipments WHERE shipping_status = 'New'", null, 0, Shipments.class, 0);
                dataShipmentsStatus.put("New", listShipments.size());
                listShipments = (List<Shipments>) clipServices.findAllByProperty("FROM Shipments WHERE shipping_status = 'Processing'", null, 0, Shipments.class, 0);
                dataShipmentsStatus.put("Processing", listShipments.size());
                listShipments = (List<Shipments>) clipServices.findAllByProperty("FROM Shipments WHERE shipping_status = 'In Transit'", null, 0, Shipments.class, 0);
                dataShipmentsStatus.put("In Transit", listShipments.size());
                listShipments = (List<Shipments>) clipServices.findAllByProperty("FROM Shipments WHERE shipping_status = 'Delivered'", null, 0, Shipments.class, 0);
                dataShipmentsStatus.put("Delivered", listShipments.size());

                //Lay thong tin billing
//                List<Wallets> listWallets = (List<Wallets>) clipServices.findAllByProperty("FROM Wallets", null, 0, Shipments.class, 0);
//                for (Wallets w : listWallets) {
//                    
//                }
                //Lay thong tin last shipments
                List<Shipments> listLastShipments = (List<Shipments>) clipServices.findAllByProperty("FROM Shipments ORDER BY created_at", null, 0, Shipments.class, 0);
                if (listShipments.size() > 0) {
...
//                    }
                ...
                List<Transfer> listLastTransactions = (List<Transfer>) clipServices.findAllByProperty("FROM Transfer ORDER BY created_at", null, 0, Shipments.class, 0);
                if (listShipments.size() > 0) {
